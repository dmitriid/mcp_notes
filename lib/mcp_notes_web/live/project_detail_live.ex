defmodule McpNotesWeb.ProjectDetailLive do
              ...
              <div class="bg-white p-4 sm:p-6 border border-gray-200 rounded-lg">
                <div class="flex flex-col sm:flex-row sm:items-center space-y-4 sm:space-y-0 sm:space-x-4 mb-4">
                  <div class="w-12 h-12 sm:w-16 sm:h-16 bg-purple-100 rounded-2xl flex items-center justify-center flex-shrink-0">
                    <Icon.icon name={Navigation.get_project_icon(@project)} class="w-6 h-6 sm:w-8 sm:h-8 text-purple-600" />
                  </div>
                  <div class="min-w-0 flex-1">
                    ...
                    <% else %>
                      <div class="flex items-center justify-between relative">
                        <h2 class="text-2xl sm:text-3xl font-bold text-gray-900" class={if @deleting_project, do: "truncate pr-32", else: "truncate"}>
                          <%= @project.name %>
                        </h2>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-6 text-sm text-gray-500 space-y-2 sm:space-y-0">
                  ...
                  <div class="flex items-center space-x-2">
                    ...
                    <span>Last updated <%= format_date(@project.last_note_updated_at || @project.updated_at) %></span>
                  </div>
                </div>
              </div>
            </div>
                    ...
                    <div class="flex items-start justify-between">
                      <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between relative mb-1">
                          <h3 class="text-lg sm:text-xl text-gray-900 font-semibold" class={if @deleting_note == note.id, do: "truncate pr-32", else: "truncate"}>
                            <%= note_title(note) %>
                          </h3>
                          <div class="flex items-center space-x-1 absolute right-0">
                            <button
                        ...
                        <div class="text-gray-600 leading-relaxed text-sm sm:text-base">
                          ...
                          <% else %>
                            <%= if @expanded_note == note.id do %>
                              <div class="whitespace-pre-wrap pt-4 text-gray-800 border-t border-gray-100 mt-4">
                                <%= note.content %>
                              </div>
                            <% else %>
                              <%= note_preview(note) %>
  ...
  def handle_event("save_note", %{"note_id" => id, "content" => content}, socket) do
    case McpNotes.Notes.Note.update_note(%{note_id: id, content: content}) do
      {:ok, _} ->
        {:ok, project} = Ash.get(Project, socket.assigns.project.id, load: [:notes, :last_note_updated_at])
        {:ok, recent_projects} = Project.list_recent_projects()
        {:noreply, assign(socket, project: project, recent_projects: recent_projects, editing_note: nil, edit_content: "")}
  ...
  def handle_event("confirm_delete_note", %{"id" => id}, socket) do
    ...
    case Ash.destroy(note) do
      :ok ->
        {:ok, project} = Ash.get(Project, socket.assigns.project.id, load: [:notes, :last_note_updated_at])
        {:ok, recent_projects} = Project.list_recent_projects()
        {:noreply, assign(socket, project: project, recent_projects: recent_projects, deleting_note: nil)}
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Failed to delete note") |> assign(:deleting_note, nil)}
    end
  end
  ...
  def handle_event("add_note", %{"content" => content}, socket) do
    case McpNotes.Notes.Note.add_note_to_project(%{project_name: socket.assigns.project.name, content: content}) do
      {:ok, _} ->
        {:ok, project} = Ash.get(Project, socket.assigns.project.id, load: [:notes, :last_note_updated_at])
        {:ok, recent_projects} = Project.list_recent_projects()
        {:noreply, assign(socket, project: project, recent_projects: recent_projects, show_add_note: false, new_note_content: "")}
  ...
  def handle_event("edit_project", _, socket) do
    {:noreply, assign(socket, editing_project: true, edit_project_name: socket.assigns.project.name)}
  end
  ...
  def handle_event("save_project", %{"name" => name}, socket) do
    case Ash.update(socket.assigns.project, %{name: name}) do
      {:ok, project} ->
        {:ok, projects} = Project.list_full_projects()
        {:ok, recent_projects} = Project.list_recent_projects()
        {:noreply, assign(socket, project: project, projects: projects, recent_projects: recent_projects, editing_project: false, edit_project_name: "")}
  ...
  def handle_event("confirm_delete_project", _, socket) do
    case Ash.destroy(socket.assigns.project) do
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Failed to delete project") |> assign(:deleting_project, false)}
    end
  end
